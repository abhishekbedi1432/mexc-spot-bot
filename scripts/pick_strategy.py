...
from bot.data import load_candles as _load_candles
...
def _run_for_symbols(
    ...
    iv = interval or config.KLINE_INTERVAL
    all_metrics: list[BacktestMetrics] = []
    chosen: dict[str, str] = {}
    missing: list[str] = []

    label = f"min_confidence={min_confidence:.1f}" if min_confidence > 0 else "baseline (no filter)"
    print(f"\nRunning backtest [{label}] [{iv}]: {len(symbols)} pair(s) × {len(STRATEGIES)} strategies\n")

    for symbol in symbols:
        candles = _load_candles(symbol, iv)
        if not candles:
    ...
    if save and chosen:
        ...
        merged["_note"] = (
            ...
            f"interval={iv}, min_confidence={min_confidence:.1f}. Do not edit manually."
        )
...
def _run_comparison(symbol_arg) -> int:
    ...
    base = _run_for_symbols(symbols, min_confidence=0.0, save=False, interval=None)
    filt = _run_for_symbols(symbols, min_confidence=0.3, save=True, interval=None)  # save filtered
...
def main() -> int:
    ...
    parser.add_argument(
        "--interval", default=config.KLINE_INTERVAL,
        choices=["1m", "3m", "5m", "15m", "30m", "1h", "2h", "4h"],
        help=f"Kline interval (default: {config.KLINE_INTERVAL})",
    )
    parser.add_argument(
    ...
    _run_for_symbols(symbols, min_confidence=args.min_confidence, save=True, interval=args.interval)
    return 0
